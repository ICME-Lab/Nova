use crate::{
  ...
  traits::{Engine, Group, TranscriptEngineTrait},
};
...
impl<E: Engine> SumcheckProof<E> {
  ...
  where
    F: Fn(&E::Scalar, &E::Scalar, &E::Scalar) -> E::Scalar,
  {
    let mut e = *claim;
  ...
  pub fn prove_cubic_batched<F>(
    ...
    num_rounds: usize,
    poly_vec: (
      &mut Vec<&mut MultilinearPolynomial<E::Scalar>>,
      &mut Vec<&mut MultilinearPolynomial<E::Scalar>>,
      &mut MultilinearPolynomial<E::Scalar>,
    ),
    coeffs: &[E::Scalar],
    comb_func: F,
    transcript: &mut E::TE,
    ...
    F: Fn(&E::Scalar, &E::Scalar, &E::Scalar) -> E::Scalar,
  {
    ...
    for _j in 0..num_rounds {
      let mut evals: Vec<(E::Scalar, E::Scalar, E::Scalar)> = Vec::new();
...
#[serde(bound = "")]
pub struct LayerProofBatched<E: Engine> {
  proof: SumcheckProof<E>,
  claims_prod_left: Vec<E::Scalar>,
  claims_prod_right: Vec<E::Scalar>,
}

impl<E: Engine> LayerProofBatched<E> {
  pub fn verify(
    &self,
    claim: E::Scalar,
    num_rounds: usize,
...
#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(bound = "")]
pub(crate) struct ProductArgumentBatched<E: Engine> {
  proof: Vec<LayerProofBatched<E>>,
}

impl<E: Engine> ProductArgumentBatched<E> {
  pub fn prove(
    poly_vec: &[&MultilinearPolynomial<E::Scalar>],
    transcript: &mut E::TE,
  ) -> Result<(Self, Vec<E::Scalar>, Vec<E::Scalar>), NovaError> {
    let mut prod_circuit_vec: Vec<_> = (0..poly_vec.len())
      .map(|i| ProductArgumentInputs::<E::GE>::new(poly_vec[i]))
      .collect();
    ...
    for layer_id in (0..num_layers).rev() {
      ...
      let comb_func_prod = |poly_A_comp: &E::Scalar,
                            poly_B_comp: &E::Scalar,
                            poly_C_comp: &E::Scalar|
       -> E::Scalar { *poly_A_comp * *poly_B_comp * *poly_C_comp };

      let mut poly_A_batched: Vec<&mut MultilinearPolynomial<E::Scalar>> = Vec::new();
      let mut poly_B_batched: Vec<&mut MultilinearPolynomial<E::Scalar>> = Vec::new();
  ...
  pub fn verify(
    ...
    for (num_rounds, i) in (0..num_layers).enumerate() {
      ...
      let eq: E::Scalar = (0..rand.len())
        .map(|i| {
          rand[i] * rand_prod[i] + (E::Scalar::ONE - rand[i]) * (E::Scalar::ONE - rand_prod[i])
        })
        .fold(E::Scalar::ONE, |acc, item| acc * item);
      let claim_expected: E::Scalar = (0..claims_prod_vec.len())
